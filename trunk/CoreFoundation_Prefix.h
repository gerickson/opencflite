...
#if !defined(COREFOUNDATION_PREFIX_H)
#define COREFOUNDATION_PREFIX_H 1
...
CF_INLINE bool OSAtomicCompareAndSwapPtr(void *oldp, void *newp, void *volatile *dst)
{ 
    void *original = InterlockedCompareExchangePointer(dst, newp, oldp);
    return (original == oldp);
}

CF_INLINE BOOL OSAtomicCompareAndSwapLong(long oldl, long newl, long volatile *dst) 
{ 
...
CF_INLINE BOOL OSAtomicCompareAndSwapPtrBarrier(void *oldp, void *newp, void * volatile *dst) 
{ 
    void *original = InterlockedCompareExchangePointer(dst, newp, oldp);
    return (original == oldp);
}

CF_INLINE int32_t OSAtomicDecrement32Barrier(volatile int32_t *dst)
{
    return InterlockedDecrement((volatile long *)dst);
}

CF_INLINE int32_t OSAtomicIncrement32Barrier(volatile int32_t *dst)
{
    return InterlockedIncrement((volatile long *)dst);
}

CF_INLINE int32_t OSAtomicIncrement32(volatile int32_t *dst)
{
    return (unsigned int)InterlockedIncrement((volatile long*)dst);
}

CF_INLINE int32_t OSAtomicDecrement32(volatile int32_t *dst)
{
    return (unsigned int)InterlockedDecrement((volatile long *)dst);
}
    
CF_INLINE int32_t OSAtomicAdd32Barrier(int32_t theAmount, volatile int32_t *dst)
{
    return (unsigned int)InterlockedExchangeAdd((volatile long*)dst, theAmount);
}

CF_INLINE BOOL OSAtomicCompareAndSwap32Barrier(int32_t oldl, int32_t newl, int32_t volatile *dst) 
{ 
